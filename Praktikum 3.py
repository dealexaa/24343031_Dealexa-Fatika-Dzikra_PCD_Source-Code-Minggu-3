#24343031_DEALEXA FATIKA DZIKRA_PRAKTIKUM MINGGU 3
# ============================================
...
def demonstrate_image_registration():
    ...
    # ===============================
    # 2. Create Moving Image
    # (Apply known transformation)
    # ===============================
    M_combined = cv2.getRotationMatrix2D((128, 128), 15, 0.9)
    M_combined[0, 2] += 20   # Translation X
    M_combined[1, 2] += 15   # Translation Y
    ...
    # ===============================
    # 3. Registration (Inverse Transform)
    # ===============================
    M_inverse = cv2.invertAffineTransform(M_combined)
    registered_img = cv2.warpAffine(moving_img, M_inverse, (256, 256))
    ...
    mse_before, psnr_before = compute_metrics(ref_img, moving_img)
    mse_after, psnr_after = compute_metrics(ref_img, registered_img)
    
    error_before = np.abs(ref_img.astype(float) - moving_img.astype(float))
    error_after = np.abs(ref_img.astype(float) - registered_img.astype(float))
    ...
    axes[0, 1].set_title(f"Moving Image\nMSE: {mse_before:.1f}\nPSNR: {psnr_before:.1f} dB")
    axes[0, 1].axis('off')
    
    axes[0, 2].imshow(registered_img, cmap='gray')
    axes[0, 2].set_title(f"Registered Image\nMSE: {mse_after:.1f}\nPSNR: {psnr_after:.1f} dB")
    axes[0, 2].axis('off')
    
    # Row 2: Error Maps
    im1 = axes[1, 0].imshow(error_before, cmap='hot', vmin=0, vmax=100)
    axes[1, 0].set_title("Error Before Registration")
    axes[1, 0].axis('off')
    plt.colorbar(im1, ax=axes[1, 0], fraction=0.046, pad=0.04)
    
    im2 = axes[1, 1].imshow(error_after, cmap='hot', vmin=0, vmax=100)
    axes[1, 1].set_title("Error After Registration")
    axes[1, 1].axis('off')
    ...
    print("\nAnalisis Registrasi:")
    print("="*50)
    print(f"Sebelum Registrasi  →  MSE: {mse_before:.2f}, PSNR: {psnr_before:.2f} dB")
    print(f"Setelah Registrasi  →  MSE: {mse_after:.2f}, PSNR: {psnr_after:.2f} dB")
    
    improvement = mse_before - mse_after
    print(f"Perbaikan Error (ΔMSE): {improvement:.2f}")
